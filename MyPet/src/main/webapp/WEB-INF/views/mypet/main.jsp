...
<div id="content">
	...
	<span class="main-title">반려동물과 함께하는 행복플러스</span> <span
		class="glyphicon glyphicon-plus"></span> <input type="button"
		class="btn main-btn" value="입양하러가기"
		onclick="location.href='/mypet/adoption/list.action'">
	<div class="review">
		...
		<div class="review-box">
			<c:forEach items="${rlist }" var="rlist" begin="0" end="4">
				<div class="review1">
					...
					<img src="/mypet${arr[0] }"
						onclick="location.href='/mypet/board/adoptionreviewview.action?seqAdoptionReview=${rlist.seqAdoptionReview}'">
					<span class="review-title"
						onclick="location.href='/mypet/board/adoptionreviewview.action?seqAdoptionReview=${rlist.seqAdoptionReview}'">${rlist.title }</span>
					<span class="review-content">${rlist.content }</span>
				</div>
			</c:forEach>
		</div>
		<input type="button" class="btn main-btn2" value="더 보러가기"
			onclick="location.href='/mypet/board/adoptionreviewlist.action'">
	</div>

	<div class="talent">
		<span class="stat-title">마이펫을 방문한 스타들</span> <img
			src="/mypet/resources/images/talent/1.jpg" id="img1" class="img">
...
<script>
	...
	function changeImg() {
		$("#slideImg").attr("src",
				"/mypet/resources/images/intro/" + imgAry[imgIdx] + ".jpg");
		imgIdx++;
