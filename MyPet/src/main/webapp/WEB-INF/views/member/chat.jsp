...
<body>
		...
		<div class="card">
			...
			<div class="col-sm-12 col-md-9"
				...
				<div class="info">
					...
					<div>
						<img src="/mypet/resources/images/chat/outimg.png" id="chatout"
							name="chatout" onclick="closeSocket();">
					</div>
				...
				<button type="button" onclick="openSocket();"
					class="btn btn-default" id="intochat">채팅 시작</button>
				...
				<div id="messages" class="messages"
					style="width: 100%; height: 360px; padding: 15px; overflow: auto;">
					<%-- ${list.size()} --%>


					<c:forEach items="${list}" var="chatMessageDto">

						<!-- 보낸 사람이 관리자일 때(sender == 1) -->
				...
				<div class="send">
					<table class="table table-bordered">
						<tr>
							...
							<td><input type="text" id="messageinput"
								placeholder="대화내용을 입력하세요." class="chatfooter"></td>

							<td style="border-bottom-right-radius: 16px;"><input
								type="button" name="sendBtn" class="sendBtn btn" id="sendBtn"
								value="보내기" onclick="send();"></td>
						</tr>
	...
	<script type="text/javascript">
		...
		function writeResponse(text) {
			...
			if (text.indexOf("님") != -1) {
				messages.innerHTML += "<div style='font-weight: bold; margin-bottom:8px !important; text-align: right'>"
						+ text + "</div>";

			} else if (text == "마이펫 채팅에 연결 되었습니다.") {
				messages.innerHTML += "<div style='margin-bottom:15px; text-align: center'>"
						+ text + "</div>";

			} else if (text.indexOf("마이펫") != -1) { //마이펫 단어가 있으면(차후에 수정해보기)
				messages.innerHTML += "<div style='font-weight: bold; margin-bottom:8px !important; text-align: left'>"
						+ text.substr(0, 3)
						+ "</div>"
						+ "<div style='margin-bottom: 15px; text-align: left; font-size: 14px; background-color: #F4F4F4; padding: 8px 8px; border-radius: 12px; width: fit-content; max-width: 260px; float: left;'>"
						+ text.substr(3)
						+ "</div> <div style='clear: both;'></div>";

			} else if (text.indexOf("마이펫") == -1) {
				messages.innerHTML += "<div style='margin-bottom: 15px; text-align: right; font-size: 14px; background-color: #F4F4F4; padding: 8px 8px; border-radius: 12px; width: fit-content; max-width: 260px; float: right;'>"
